...
i=0
for core_repo_suffix in "${repo_suffixes[@]}"
do
    ret_code="$(curl -sSLIo /dev/null -w '%{http_code}' "https://steamdeck-packages.steamos.cloud/archlinux-mirror/core${core_repo_suffix}/os/x86_64/core${core_repo_suffix}.db")"
    if [[ "$ret_code" == '200' ]]
    then
        for repo in "${repos[@]}"
        do
